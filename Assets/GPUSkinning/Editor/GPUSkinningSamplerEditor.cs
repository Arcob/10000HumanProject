...
public class GPUSkinningSamplerEditor : Editor 
{
    ...
    public override void OnInspectorGUI ()
	{
        ...
        //从Animation组件中获取AnimationClip列表（Animator组件无用）
            ...
            if (GUILayout.Button("Preview/Edit"))
            {
                ...
                else
                {
                    if (rt == null && !EditorApplication.isPlaying)
                    {
                        ...
                        //Object Camera hideFlags设置为隐藏和不保存，无法在hierarchy视图查看
                        //Camera用于绘制RenderTexture
    ...
    /// <summary>
    /// 在Inspector视图显示骨骼层级关系
    /// </summary>
    /// <param name="bone"> 骨骼 </param>
    /// <param name="indentLevel"> 骨骼父子层级数 </param>
    ...
    private void UpdateHandler()
    {
        ...
        //脚本正在编译时，无法选中GPUSkinningSampler绑定的游戏对象（可能会出错）
        ...
        if(!sampler.isSampling && sampler.IsSamplingProgress())
        {
            ...
            //所有动画片段Sample完毕，但是所有网格未Sample完毕
            else if (++sampler.samplingMeshIndex < sampler.samplingMeshCount)
            {
                sampler.anim = null;
                sampler.samplingClipIndex = 0;
                sampler.StartSample();
            }
            //所有动画片段、网格均Sample完毕
            else
        ...
        if (sampler.isSampling)
        {
            ...
            string msg = sampler.animClip.name + "(" + (sampler.samplingClipIndex) + "/" + sampler.animClips.Length + ")_Mesh " + sampler.samplingMeshIndex + ")";
