    ...
    public class Simulator
    {
        ...
        private class Worker
        {
            ...
            internal void step(object obj)
            {
                ...
                {
                    if (!Simulator.Instance.agents_[agentNo].isActive) continue;
                    Simulator.Instance.agents_[agentNo].setPrefVelocity();
         ...
         * agent.</param>
         */
        // 简单添加Agent的方法，仅使用Position，agent的其它参数与defaultAgent相同
        public int addAgent(Vector3 position)
        {
            // dafaultAgent未设置，Agent添加失败
            ...
            agent.position_ = RVOMath.V3ToV2(position);
        ...
        /// <summary>
        /// fzy add:添加agent,设置goal,主要构建函数
        /// </summary>
        /// <param name="position"></param>
        /// <param name="goal"></param>
        /// <returns></returns>
        public int addAgent(Vector3 position, Vector3 _goal, float _positionY, 
            GPUSkinningController _gpuSkinningController, bool _isActive)
        {
            // dafaultAgent未设置，Agent添加失败
            if (defaultAgent_ == null)
            {
            ...
            // fzy add：
            agent.radius_default = agent.radius_;
            agent.origin = position;
            agent.goal = _goal;
            agent.positionY = _positionY;
            agent.position_v3 = position;
            agent.rotation = Quaternion.identity;
            agent.gpuSkinningController = _gpuSkinningController;
         ...
         * in counterclockwise order. 逆时针 </param>
         *
        ...
        {
            if (obstacleNo < 0 || obstacleNo >= obstacles_.Count)
                return;
            Obstacle firstObstacle = obstacles_[obstacleNo];
            Obstacle nextObstacle = firstObstacle.next_;
            List<Obstacle> obstaclesToRemove = new List<Obstacle>();
            obstaclesToRemove.Add(firstObstacle);
            while(nextObstacle != firstObstacle)
            {
                obstaclesToRemove.Add(nextObstacle);
                nextObstacle = nextObstacle.next_;
            }
        ...
        public void Clear()
        {
            ...
            if (MySceneManager.isThread)
                SetNumWorkers(ThreadManager.Instance.threadNum - 1);
            else
                SetNumWorkers(0);
        ...
        public void SetWorkers()
        {
            if (workers_ == null)
            {
                ...
                Debug.Log(workers_.Length);

                for (int block = 0; block < workers_.Length; ++block)
                {
                    doneEvents_[block] = new ManualResetEvent(false);
                    // 设置worker线程管理的Agent范围索引（算头不算尾）
                    workers_[block] = new Worker(block * getNumAgents() / workers_.Length, (block + 1) * getNumAgents() / workers_.Length, doneEvents_[block]);
                }
            }
            ThreadManager.Instance.SetThreadJobWithThreadID(kdTree_.buildAgentTree, ThreadManager.Instance.threadNum-1);
            for(int block = 0; block < workers_.Length; ++block)
            {
                ThreadManager.Instance.SetThreadJobWithThreadID(workers_[block].step, block);
                ThreadManager.Instance.SetThreadJobWithThreadID(workers_[block].update, block);
            }
        }
        #endregion
        ...
        public float doStep()
        {
            ...
            Profiler.BeginSample("Build Agent Tree");
            //ThreadPool.QueueUserWorkItem(kdTree_.buildAgentTree_ThreadPool);
            kdTree_.buildAgentTree();
            Profiler.EndSample();

            Profiler.BeginSample("worker step");
            for (int block = 0; block < workers_.Length; ++block)
            {
                // 线程完成信号重置
            ...
            // 等待所有线程执行完成step工作->获取所有返回信号doneEvents_
            // 阻塞
            WaitHandle.WaitAll(doneEvents_);
            Profiler.EndSample();

            Profiler.BeginSample("worker update");
            for (int block = 0; block < workers_.Length; ++block)
            {
                // 线程完成信号重置
                doneEvents_[block].Reset();
            ...
            // 等待所有线程执行完成update工作->获取所有返回信号doneEvents_
            // 阻塞
            WaitHandle.WaitAll(doneEvents_);
            Profiler.EndSample();

            // 计算并返回step时间
            // fzy remark：这里注意，timeStep_并非doStep的总执行时间，而是人为设定的时长
            // 如果timeStep_<doStep所消耗的时间？
        ...
        public Vector3 getAgentPosition_V3(int agentNo,float position_Y = 0.0f)
        {
            return RVOMath.V2ToV3(agents_[agentNo].position_, position_Y);
        }
        ...
        public Vector3 getAgentPrefVelocity_V3(int agentNo,float Velocity_Y = 0.0f)
        {
            return RVOMath.V2ToV3(agents_[agentNo].prefVelocity_, Velocity_Y);
        }
        ...
        public Vector3 getAgentVelocity_V3(int agentNo,float Velocity_Y = 0.0f)
        {
            return RVOMath.V2ToV3(agents_[agentNo].velocity_, Velocity_Y);
        }
        ...
        {
            agents_[agentNo].position_ = RVOMath.V3ToV2(position);
        }
        ...
        {
            agents_[agentNo].prefVelocity_ = RVOMath.V3ToV2(prefVelocity);
        }
        ...
        {
            agents_[agentNo].velocity_ = RVOMath.V3ToV2(velocity);
        }
