...
Shader "SH/LookupTableFourWarp" {
	...
	CGINCLUDE
		...
		bool invalidIndex(float4 coord){
			return coord.x == 1 && coord.y == 1 && coord.z == 1 && coord.w == 1;
		}
		...
		float4  frag (v2f i) : COLOR
		{
			...
			if(invalidIndex(coodU1) && invalidIndex(coodV1) && invalidIndex(coodA1))
			...
			// |   |
			// A - B
			float2 offsetA, offsetB, offsetC, offsetD;
			...
			outputA = tex2D(_MainTex, offsetA);
			outputB = tex2D(_MainTex, offsetB);
			outputC = tex2D(_MainTex, offsetC);
			outputD = tex2D(_MainTex, offsetD);
			output = outputA * alphaA + outputB * alphaB + outputC * alphaC + outputD * alphaD;
			 // return float4(offsetA.y, 0, 0, 1);
			// return output;
			return outputA;
			
			// ----------------- multi sampling ---------------------

			float2 q11A, q12A, q21A, q22A, r1A, r2A;
			float2 q11B, q12B, q21B, q22B, r1B, r2B;
			float2 q11C, q12C, q21C, q22C, r1C, r2C;
			float2 q11D, q12D, q21D, q22D, r1D, r2D;
